<template>
  <div v-if="isOpen" class="custom-modal">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div v-if="userInfo">
            ...
            <!-- <div class="user-info" v-if="userCourse.length > 0">
              <div class="label"><strong>Các khóa học:</strong>
              </div>
              <div class="value">{{ userCourse.map(course => course.name).join(', ') }}</div>
            </div> -->
            <div class="user-info" v-if="userCourse.length > 0">
              <div class="label">
              ...
              <div class="value">
                <div class="row">
                  <div class="col-12 mb-2" v-for="course in userCourse" :key="course.id">
                    <button class="btn btn-link text-decoration-none text-primary" @click="navigateToCourse(course.id)">
                      {{ course.name }}
                    </button>
                  </div>
                </div>
    ...
    <UpdateUserModal :isOpen="isUpdateModalOpen" :userInfo="userInfo" @update:isOpen="isUpdateModalOpen = false"
...
const fetchUserInfo = async () => {
  try {
    ...
    if (userInfo.value.courses?.body?.data) {
      userCourse.value = userInfo.value.courses.body.data;
    } else {
      console.warn("không tìm thấy khóa học cho user.");
    }
    console.log("userCourse", userCourse.value);
  } catch (error) {
...
// Computed property to determine if the user is a mentor or teacher
